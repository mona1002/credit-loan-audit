      ...
      <div class="search">
        <ul>
          <li>
            <p>
              <label> 进件编号： </label>
              <el-input v-model="params.userCode" placeholder="请输入进件编号"></el-input>
            </p>
            <p>
              <label> 发送日期： </label>
            ...
            <p>
              <label> 发送状态： </label>
              <el-select v-model="params.operOrgCodes" placeholder="请选择">
  ...
  export default {
    ...
    methods: {
      inquire(pam) { //查询列表    基础接口-5.分页查询请求错误数据
        this.post("/controller/querySendErrorByPage", pam).then(res => {
      ...
      resend() {
 this.post("/sendErrorController/sendErrorMsg", pam).then(res => {
          if (res.statusCode == 200) {
            this.tableData = res.data.recordList;
            // this.totalRecord = res.data.totalRecord; //总记录数
            // res.data. totalPage // 总页数
