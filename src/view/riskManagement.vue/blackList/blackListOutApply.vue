...
<template>
  <div class="taskWatting main-div">
        ...
        <el-form :model="addObj" :rules="addRules" ref="addObj">
          <div class="bfc">
            <el-form-item label="黑名单类型：" class="fl width_50Per" :label-width='label_115'>
              {{addObj.blackListTypeTxt}}
              <i class="el-icon-search addIcon_search" @click="getAddInf"></i>
            </el-form-item>
            <el-form-item label="加黑类型：" v-if="addObj.blackListType" class="fl width_50Per" :label-width='label_115'>
              {{addObj.inReasonsTxt}}
            </el-form-item>
          </div>
          <div class="bfc">
            <el-form-item label="客户名称：" class="fl width_50Per" :label-width='label_115' v-if="addObj.blackListType">
              {{addObj.blackCustName}}
            </el-form-item>
            <el-form-item label="证件号码：" class="fl width_50Per" v-if="addObj.blackListType" :label-width='label_115'>
              {{addObj.blackCertCode}}
            </el-form-item>
          </div>
          <div class="bfc">
            <el-form-item label="电话：" class="fl width_50Per" v-if="addObj.blackListType" :label-width='label_115'>
              {{addObj.blackPhone}}
            </el-form-item>
            <el-form-item label="地址：" class="fl width_50Per" v-if='addObj.blackListType' :label-width='label_115'>
              {{addObj.blackAddress }}
            </el-form-item>
          </div>
          <div class="bfc">
            <el-form-item label="单位名称：" class="fl width_50Per" :label-width='label_115' v-if="addObj.blackListType">
              {{addObj.blackCompany }}
            </el-form-item>
            <el-form-item label="申请人：" class="fl width_50Per" :label-width='label_115' v-if="addObj.blackListType">
              {{addObj.appPerCode}}
            </el-form-item>
          </div>
          <el-form-item class="mr height_60" label="转入申请说明：" v-if='addObj.blackListType' :label-width="label_115">
            <el-input v-model="addObj.blackRemark" type="textarea" resize='none' :rows="2" disabled></el-input>
          </el-form-item>
          <el-form-item class="mr height_60" label="转出申请说明：" v-if='addObj.blackListType' :label-width="label_115" prop="outBlackRemark">
            <b class="hint_word Working_input" v-show="addObj.outBlackRemark&&addObj.outBlackRemark.length>=300">
              输入长度不能超过300</b>
            <el-input v-model="addObj.outBlackRemark" :maxlength='300' type="textarea" resize='none' :rows="2"></el-input>
          </el-form-item>
    ...
    <!-- 添加弹窗-搜索按钮 Height_600-->
    <!-- <div class=""> -->
    <el-dialog title="请选择一条信息" :visible.sync="addInfShow" width='1000px' height='900' :modal="false ">
      <el-row class="row row1" type="flex">
        <el-col :span="6" >
          <span class="dis_inBlock label_85 text_align_right">黑名单类型：</span>
          <el-select v-model="addSearchForm.blackListType" placeholder="请选择">
            <el-option v-for="item in listType" :key="item.value" :label="item.label" :value="item.value">
            </el-option>
          </el-select>
        </el-col>
        <el-col :span="6" class="search-item">
          <span>客户名称：</span>
          <el-input v-model.trim="addSearchForm.blackCustName" @keyup.enter.native='Rsearch' placeholder="请输入客户名称"></el-input>
        </el-col>
        <el-col :span="6" class="search-item">
          <span>证件号码：</span>
          <el-input v-model.trim="addSearchForm.blackCertCode" @keyup.enter.native='Rsearch' placeholder="请输入证件号码"></el-input>
        </el-col>
      </el-row>
      <el-row class="row row1" type="flex">
        <el-col :span="6" class="search-item">
          <span class="dis_inBlock label_85 text_align_right">电话：</span>
          <el-input v-model.trim="addSearchForm.blackPhone" @keyup.enter.native='Rsearch' placeholder="请输入电话"></el-input>
        </el-col>
        <el-col :span="6" class="search-item">
          <span>单位名称：</span>
          <el-input v-model.trim="addSearchForm.blackCompany" @keyup.enter.native='Rsearch' placeholder="请输入单位名称"></el-input>
        </el-col>
        <el-col :span="6" class="search-item">
      ...
      <div class="height_300">
        <el-table :data="addTableData" style="width: 100%" height="300" highlight-current-row border @row-click='getAddCurrentRow'>
          <el-table-column type="index" label=序号 width="55">
          </el-table-column>
          <el-table-column prop="blackListTypeTxt" label="黑名单类型" width="100">
          </el-table-column>
      ...
      <div class="page">
        <el-pagination @size-change="addSearchSizeChange" @current-change="addSearchCurrentChange" :current-page="addSearchPage.pageNum"
          :page-sizes="[10, 20,50]" :page-size="addSearchPage.pageSize" layout="total, sizes, prev, pager, next, jumper"
          :total="addSearchTotalRecord">
        </el-pagination>
      </div>
      <div slot="footer" class="dialog-footer">
...
<script>
  export default {
    ...
    methods: {
      ...
      addRsearch() {
        if (JSON.stringify(this.addSearchForm) == '{}') {
          this._error('请输入查询条件！')
          return
        }
          this.AddCurrentRow={};
        // 查黑名单库
        this.post('/blackAndGrey/blackListInfos', {
          param: this.addSearchForm,
          pageParam: this.addSearchPage
        }).then(res => {
          if (res.statusCode == 200) {
            this.addTableData = res.data.recordList;
            this.addSearchTotalRecord = res.data.totalRecord;
          } else {
            this.addTableData = [];
          }
        })
      ...
      addSearchAdd() {
        ...
        this.addObj = this.AddCurrentRow;
      ...
      addSearchSizeChange(val) {
        this.addSearchPage.pageNum = 1; //	页码
        this.addSearchPage.pageSize = val; //	条数
        this.addRsearch();
      },
      ...
      addSearchCurrentChange(val) {
        this.addSearchPage.pageNum = val; //	条数
        this.addRsearch();
      },
      getCurrentRow(val) {
        this.currentRow = val;
      ...
      addBlackList(formName) {
        ...
        this.blackbtnWord = '添加中';
            ...
            this.post('/blackAndGrey/blackListOutApp', {
             listId: this.addObj.id,
              blackRemark: this.addObj.outBlackRemark, //申请说明
              appPerCode: this.userInf.userCode //	申请人
            }).then(res => {
              if (res.statusCode == 200) {
                this._succe(res.msg);
      ...
      delAndSubmit(proType) {
        ...
        this.$confirm(title, '提示', {
          ...
          if (proType == 'del') { //删除
            this.Delete('/blackAndGrey/blackListOutApp/' + this.currentRow.id).then(res => {
              if (res.statusCode == 200) {
                this._succe(res.msg)
                this.inquire();
              } else {
            ...
            this.get('/blackAndGrey/submitOutApp/' + this.currentRow.id).then(res => {
              if (res.statusCode == 200) {
                this._succe(res.msg)
                this.inquire();
              } else {
